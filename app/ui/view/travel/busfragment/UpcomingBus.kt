...
class UpcomingBus : Fragment() {
                    ...
                    ApiStatus.SUCCESS -> {
                        ...
                        it.data?.let { users ->
                            users.body()?.let { response ->
                                ...
                                hitApiForCancelTicket(cancellationReq)
    ...
    private fun hitApiForCancelTicket(req:BusTicketCancelReq){
                        ...
                        it.data?.let { users ->
                            users.body()?.let { response ->
                                ...
                                if(BusTicketConsListClass.startDate!!.isNullOrEmpty()&& BusTicketConsListClass.endDate!!.isNullOrEmpty()) {
                                    (activity as? MyBookingBusActivity)?.hitApiForBookingList(
                                        BusTicketConsListClass.startDate,
                                        BusTicketConsListClass.endDate
                                    )
                                }
