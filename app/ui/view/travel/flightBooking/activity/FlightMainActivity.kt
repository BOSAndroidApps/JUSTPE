...
class FlightMainActivity : AppCompatActivity() , FlightSpecialOfferAdapter.setClickListner {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this,
            TravelViewModelFactory(
                TravelRepository(
                    RetrofitClient.apiAllTravelAPI,
                    RetrofitClient.apiBusAddRequestlAPI)
            )
    ...
    private fun setclickListner(){
        ...
        binding.departureDatelayout.setOnClickListener {
           ...
           val  dpd = DatePickerDialog(this,  R.style.MyDatePickerDialogTheme, DatePickerDialog.OnDateSetListener { _, selectedYear, selectedMonth, selectedDay ->
                firstDateCalendar = c
                val formattedDate = String.format("%02d/%02d/%04d", selectedDay, selectedMonth + 1, selectedYear)
                binding.datemonthdeparture.text=formatDate1(formattedDate)
                binding.dayyear.text=formatDate2(formattedDate)
                travelDate=  String.format("%02d/%02d/%04d", selectedMonth + 1,selectedDay, selectedYear)


            }, year, month, day)

            dpd!!.setOnShowListener {
                dpd!!.getButton(DatePickerDialog.BUTTON_POSITIVE)?.setTextColor(ContextCompat.getColor(this, R.color.green))
                dpd!!.getButton(DatePickerDialog.BUTTON_NEGATIVE)?.setTextColor(ContextCompat.getColor(this, R.color.red))
            }
        ...
        binding.addreturndatelayout.setOnClickListener{
            ...
            val secondDatePicker = DatePickerDialog(this, R.style.MyDatePickerDialogTheme, { _, y, m, d ->
            ...
            secondDatePicker!!.setOnShowListener {
                secondDatePicker!!.getButton(DatePickerDialog.BUTTON_POSITIVE)?.setTextColor(ContextCompat.getColor(this, R.color.green))
                secondDatePicker!!.getButton(DatePickerDialog.BUTTON_NEGATIVE)?.setTextColor(ContextCompat.getColor(this, R.color.red))
            }
        ...
        binding.searchflightlayout.setOnClickListener {
            ...
            if (mStash?.getStringValue(Constants.requestId, "").isNullOrEmpty()) {
                        ...
                        it.data?.let { users ->
                            users.body()?.let { response ->
                                ...
                                if(response.responseHeader.errorCode.equals("0000")){
                                    ...
                                    registrationNo  = ""
                                    CheckBox = false
                                    mStash.setStringValue("search_Key",response.searchKey)
    ...
    fun getTripInfoList():List<TripInfo>{
        ...
        airlinecodeList.add(TripInfo(origin = binding.fromairportcode.text.toString(), destination = binding.toairportcode.text.toString(), traveldate = travelDate, tripId = 0))
        return airlinecodeList
    }
